...
erDiagram
    ...
    chunk_search_cache {
        INTEGER id PK "主键"
        TEXT query_hash "搜索查询哈希"
        INTEGER file_id FK "关联文件ID"
        INTEGER chunk_id FK "关联分块ID"
        REAL relevance_score "相关性评分"
        INTEGER rank_position "搜索结果排名"
        DATETIME created_at "创建时间"
    }
    ...
    file_chunks ||--o{ chunk_search_cache : "分块搜索结果缓存"
...
#### 2.2.3 chunk_search_cache 分块搜索缓存表 (v2.0新增)
缓存分块搜索结果，优化重复查询性能。

| 字段名 | 数据类型 | 约束 | 说明 | 索引 |
|--------|----------|------|------|------|
| id | INTEGER | PRIMARY KEY AUTOINCREMENT | 主键 | PRIMARY |
| query_hash | TEXT | NOT NULL | 搜索查询哈希 | INDEX |
| file_id | INTEGER | NOT NULL | 关联文件ID | FOREIGN KEY, INDEX |
| chunk_id | INTEGER | | 关联分块ID | FOREIGN KEY, INDEX |
| relevance_score | REAL | NOT NULL | 相关性评分 | |
| rank_position | INTEGER | NOT NULL | 搜索结果排名 | |
| created_at | DATETIME | NOT NULL DEFAULT CURRENT_TIMESTAMP | 创建时间 | INDEX |

#### 2.2.4 search_history 搜索历史表
...
| chunk_search_cache | ChunkSearchCacheModel | chunk_search_cache | 🔄 待实现 | v2.0新增缓存表 |
...
5. **🆕 v2.0新增: 添加分块支持设计** - 新增file_chunks和chunk_search_cache表
